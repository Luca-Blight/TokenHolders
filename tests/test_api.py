...
def test_get_token_holders():
    ...
    assert response.json()["token_holders"][0]["balance"] < response.json()["token_holders"][-1]["balance"]
    # Check if token holders are ordered by balance ascending


def test_get_token_holder():
    response = client.get("/0x742d35Cc6634C0532925a3b844Bc454e4438f44e")
    assert response.status_code == 200
    ...
    response = client.get("/0x742d35Cc6634C0532925a3b844Bc454e4438f44e?balance=true")
    assert response.status_code == 200
    assert "balance" in response.json()["token_holder"][0]
    assert "weekly_balance_change" not in response.json()["token_holder"][0]

    response = client.get("/0x742d35Cc6634C0532925a3b844Bc454e4438f44e?weekly_balance_change=true")
    assert response.status_code == 200
